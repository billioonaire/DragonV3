...
const CONTRACT_ADDRESS = "0xea68b14bca0a7ebc9461969ea68454fa7f7135f6";
...
const Main: React.FC = () => {
  ...
  const mint = async (
    ...
    let claimedFree = await contract.vipMinters(walletAddress.toString());
    let counter = await contract.freeMintCounter();

    console.log(counter.toString())
    console.log("TEST CALL" + claimedFree)

        if (claimedFree){
          result = await contract.vipMint(quantity, proof, {
          ...
          result = await contract.vipMint(quantity, proof, {
            value: ethers.utils.parseEther(
              (String(parseFloat(data[type as keyof typeof data].cost) * (quantity-1)))
            ),
          });
      ...
      case 'Whitelist':
        ...
        result = await contract.wlMint(quantity, proof, {
          value: ethers.utils.parseEther(
            (String(parseFloat(data[type as keyof typeof data].cost) * quantity))
          ),
        });
